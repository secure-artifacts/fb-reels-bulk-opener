...
// 日程本书签名称前缀，用于判断是否已存在同类书签
const CALENDAR_BK_KEYWORD = '日程本';
...
// ── DOM：设置页面 ─────────────────────────────────────────────
const sCountNum       = document.getElementById('sCountNum');
const sMinusBtn       = document.getElementById('sMinusBtn');
const sPlusBtn        = document.getElementById('sPlusBtn');
const sSpeedNormal    = document.getElementById('sSpeedNormal');
const sSpeedFast      = document.getElementById('sSpeedFast');
const togFbHome       = document.getElementById('togFbHome');
const togCalendar     = document.getElementById('togCalendar');
const togAutoBookmark = document.getElementById('togAutoBookmark');
const togUpdate       = document.getElementById('togUpdate');
...
function renderSettings() {
  sCountNum.textContent = settings.defaultCount;
  setToggle(togFbHome,       settings.preOpenFbHome);
  setToggle(togCalendar,     settings.preOpenCalendar);
  setToggle(togAutoBookmark, settings.autoBookmark);
  setToggle(togUpdate,       settings.checkUpdate);
  renderSpeedSel(settings.defaultSpeed);
}
...
function renderSpeedSel(s) {
  sSpeedNormal.className = 'set-sel-btn' + (s === 'normal' ? ' active-normal' : '');
  sSpeedFast.className   = 'set-sel-btn' + (s === 'fast'   ? ' active-fast'   : '');
}

// 设置页各控件事件
sMinusBtn.addEventListener('click', () => {
  settings.defaultCount = Math.max(1, settings.defaultCount - 1);
  sCountNum.textContent = settings.defaultCount;
  // 同步主页面当前数量
...
sPlusBtn.addEventListener('click', () => {
  settings.defaultCount = Math.min(20, settings.defaultCount + 1);
  sCountNum.textContent = settings.defaultCount;
  setCount(settings.defaultCount);
  saveSettings();
});

sSpeedNormal.addEventListener('click', () => {
  settings.defaultSpeed = 'normal';
  renderSpeedSel('normal');
  setSpeed('normal');
  saveSettings();
});
sSpeedFast.addEventListener('click', () => {
...
makeTogHandler(togFbHome,       'preOpenFbHome');
makeTogHandler(togCalendar,     'preOpenCalendar');
makeTogHandler(togAutoBookmark, 'autoBookmark');
makeTogHandler(togUpdate,       'checkUpdate');
...
function isFbProUrl(u) {
  try {
    const host = new URL(u).hostname.toLowerCase();
    return host === 'www.facebook.com' || host === 'facebook.com' ||
           host === 'web.facebook.com' || host === 'business.facebook.com';
  } catch { return false; }
}
...
// ── 自动添加日程本书签 ────────────────────────────────────────
// 检查书签树里是否已有同类（含 CALENDAR_BK_KEYWORD 或 content_calendar URL）
async function ensureCalendarBookmark(pageId, pageName) {
  if (!settings.autoBookmark || !pageId) return;
  try {
    const tree     = await chrome.bookmarks.getTree();
    const calUrl   = buildCalendarUrl(pageId);
    const keyword  = CALENDAR_BK_KEYWORD;
    ...
    function scan(nodes) {
      for (const n of nodes) {
        if (n.url) {
          if (n.url.includes('content_calendar') || (n.title && n.title.includes(keyword))) {
            found = true; return;
          }
        }
    ...
    // 没有 → 新建书签放到书签栏
    const title = pageName ? `日程本 · ${pageName}` : '日程本';
    await chrome.bookmarks.create({ title, url: calUrl });
  } catch {}
}
...
function showAutoDetect(name, pageId) {
  ...
  // 自动添加日程本书签
  ensureCalendarBookmark(pageId, name);
}
...
btnGo.addEventListener('click', async () => {
  ...
  try {
    ...
    // 根据设置决定是否加前置任务
    if (settings.preOpenFbHome) {
      const fbOpen = allTabs.some(t => t.url && isFbProUrl(t.url));
      if (!fbOpen) preTasks.push({ url: FB_HOME_URL, label: 'FB主页' });
    }
    ...
    chrome.runtime.sendMessage({ type: 'OPEN_TABS', url: detectedUrl, count, speed }, () => { window.close(); });
  }
});
